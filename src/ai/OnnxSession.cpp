...
#if !AUCLEAR_HAS_ONNXRUNTIME
    (void) modelPath; (void) inputName; (void) outputName;
...
OnnxSession::Status OnnxSession::getStatus () const { return pImpl->status; }
const std::string& OnnxSession::getLastError () const { return pImpl->lastError; }
int OnnxSession::frameSize () const { return pImpl->fSize; }
...
bool OnnxSession::runFrame (const float* in, float* out)
{
    ...
    try
    {
        auto outputs = impl.session->Run (
            Ort::RunOptions{nullptr}, &inName, &inTensor, 1, &outName, 1);
