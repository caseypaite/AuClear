...
void MeterBridge::timerCallback()
{
    ...
    {
        MeterValues fresh { display.peakL * 0.85f, display.peakR * 0.85f,
                            display.rmsL  * 0.85f, display.rmsR  * 0.85f };
        if (bus.drain (fresh))
            repaintNeeded = true;
        display = fresh;
    };
...
void MeterBridge::drawMeterBar (juce::Graphics& g, juce::Rectangle<float> bounds,
                                 float peakDb, float holdDb) const
{
    ...
    if (frac > 0.0f)
    {
        ...
        else if (frac <= amberFrac)
        {
            ...
            g.setColour (juce::Colour (kGreen));
            g.fillRoundedRectangle ({ bar.getX(), bounds.getBottom() - greenH,
                                      bar.getWidth(), greenH }, 2.0f);
            g.setColour (juce::Colour (kAmber));
            g.fillRoundedRectangle ({ bar.getX(), barY, bar.getWidth(),
                                      barH - greenH }, 2.0f);
        }
        else
        {
            ...
            g.fillRoundedRectangle ({ bar.getX(), bounds.getBottom() - greenH,
                                      bar.getWidth(), greenH }, 2.0f);
            g.setColour (juce::Colour (kAmber));
            g.fillRoundedRectangle ({ bar.getX(), bounds.getBottom() - amberH,
                                      bar.getWidth(), amberH - greenH }, 2.0f);
