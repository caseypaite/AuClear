...
void RackModuleCard::paint (juce::Graphics& g)
{
    ...
    if (selected)
        bg = juce::Colour (kBgSelect);
    else if (hovered)
        bg = juce::Colour (kBgHover);
    g.fillAll (bg);
    ...
    g.drawText (
        moduleRef->name () == "Gain" ? juce::String (moduleRef->bypassed ? "bypassed" : "active")
                                     : juce::String (),
        nameX, getHeight () / 2, nameW, getHeight () / 2 - 4, juce::Justification::centredLeft);
...
void RackModuleCard::mouseDrag (const juce::MouseEvent& e)
{
    if (!dragging)
    {
        ...
        if (std::abs (delta.getY ()) > kDragThreshold || std::abs (delta.getX ()) > kDragThreshold)
        {
            dragging = true;
            if (listener)
                listener->cardDragStarted (this, e);
        }
    }
    else
    {
        if (listener)
            listener->cardDragged (this, e);
    }
}
...
void RackModuleCard::mouseEnter (const juce::MouseEvent&)
{
    hovered = true;
    repaint ();
}
void RackModuleCard::mouseExit (const juce::MouseEvent&)
{
    hovered = false;
    repaint ();
}
