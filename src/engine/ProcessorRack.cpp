...
{
    jassert (juce::MessageManager::getInstance()->isThisTheMessageThread());
    if (index < 0 || index >= static_cast<int> (guiOrder.size())) return;
...
{
    if (index < 0 || index >= static_cast<int> (guiOrder.size())) return nullptr;
...
void ProcessorRack::retireOldModules()
{
    ...
    for (int i = 0; i < b1; ++i) retire (retireBuffer[static_cast<size_t> (s1 + i)]);
    for (int i = 0; i < b2; ++i) retire (retireBuffer[static_cast<size_t> (s2 + i)]);
...
void ProcessorRack::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
    ...
    // Input metering
    {
        ...
        v.peakR = (nch > 1) ? linearToDb (buffer.getMagnitude (1, 0, buffer.getNumSamples())) : v.peakL;
        v.rmsL  = linearToDb (buffer.getRMSLevel   (0, 0, buffer.getNumSamples()));
        v.rmsR  = (nch > 1) ? linearToDb (buffer.getRMSLevel   (1, 0, buffer.getNumSamples())) : v.rmsL;
        inMeters.push (v);
    }
...
void ProcessorRack::drainCommandQueue()
{
    ...
    while (commandQueue.pop (cmd))
    {
        switch (cmd.type)
        {
            ...
            case RackCommand::Type::None: break;
        }
    }
